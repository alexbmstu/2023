...
Библиотека <a href="https://latex.bmstu.ru/gitlab/lnh64_l0" target="_blank">lnh64 l0</a> (библиотека уровня 0) представляет собой API системного уровня, реализующего базовые функции взаимодействия с драйвером ускорительной карты микропроцессора Леонард Эйлер. Библиотека позволяет:
...
|:----------------------------------------------------------------------|-------------------------------------------------------------|------------------------------------------------:|
| <a href="https://latex.bmstu.ru/gitlab/lnh64_l0/host-lib" target="_blank">Host Lib</a> |   Управления хост-подсистемой и взаимодействие с ядрами GPC | C/C++, объектная модель | x86, g++                | gdb, Eclipse IDE |
...
Ниже приведен пример кода программы хост-подсистемы, выполняющей инициализацию и измерение тактовой частоты GPC (hello world). Код доступен по следующему адресу: <a href="https://latex.bmstu.ru/gitlab/hackathon2023/lab1/lab1" target="_blank">Леонард Эйлер lab1</a>
...
**Таблица 3 - Методы передачи и приема сообщений хост-подсистемы**
...
Доступ к микропроцессору lnh64 (Structure Processing Element) осуществляется чтением и записью в пространство памяти микропроцессора riscv64im (Computing Processing Element) в диапазоне 0x60000000 - 0x60001000. Карта памяти представлена в файле <a href="https://gitlab.com/leonhard-x64-xrt-v2/libraries/sw-kernel-lib/-/blob/main/lib/gpc_swk.h" target="_blank">gpc_swk.h</a>:

**Таблица 4. Карта памяти CPE riscv64im**
...
| 0x80000000      | 64К                        | RAM - оперативная память микропроцессора riscv64im              |
...
Микропроцессор lnh64 является 64-х разрядным, в то время как микропроцессор riscv64im является 32-х разрядным. В связи с этим для всех регистров lnh64 обеспечивается доступ как к младшей так и к старшей частям, используя 32-х разрядную шину памяти. 
