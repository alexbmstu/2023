...
- Создание файлового дискриптора для эксклюзивного доступа к символьному устройству GPC `/dev/gpc*`.
...
*   Передача опрандов и кодов операций набора команд дискретной математики DISC в микропроцессор lnh64.
...
1. **Пакетная передача**. Данный способ передачи обладает наибольшей эффектвностью, так как за цикл прямой передачи в память хост-подсистемы осуществлется передача большого пакета данных и поле данных в пакете pcie используется в максимальной степени.
...
Программная модель микропроцессора Леонард Эйлер представляет собой совоупность программно-доступных ресурсов в следующих адресных пространствах:

- Адресное пространство хост-подсистемы: 64 битное дресне пространство, в которое отображены ресурсы  подсистемы обработки графов (за исключением ресурсов SPE), ресурсы хост-подсистемы (ОЗУ, дисковые устройства),  хост-подсистемы, подсистема хранения графов, подсистемы сетевого взаимодействия узлов обработки графов.
...
|   *RV_CONFIG*                                                                                      | 0x#10                            | W                                   |                 | Регистр номера обработчика. При записы вырабатывается сигнал ap_start, читаемый со сторны GPC#                                            |
...
| done_ie                                                                                            | R/W                                 | 3               | Бит разрегения прерывания по преходу в состояние IDLE                                                                                     |
...
| *HANDLER_PAYLOAD*                                                                                              | AXI4+0x18                                                            | W                                                           |                 | Регистр данных пользовательского прерывания. Прерывание вырабатываетя автоматически при каждой записи регистра.                               |
...
Для подключение к облачной платформе необходимо получить у оргаизаторов практикума логин и начальный пароль. 
...
**Система контроля доступа**. Сформировать в хост-подсистеме и передать в SPE структуры: RESOURCE_ENABLED и RESOURCE_AVAILABLE. В структуре RESOURCE_ENABLED хранятся id ресурсов, доступ к которому разрешен (10К устройств) и время последнего доступа к ним. В структуре RESOURCE_AVAILABLE хранятся id ресурсов, имеющихся в системе  (100К устройств), доступ к которому разрешен.  По запросу пользователя выдать те устройства, которые присутствуют в обеих структурах и доступ к которым был ранее заданного момента времени. Для реализации запроса использовать команду пересечения множеств lnh_and_async(INTERSECTION,RESOURCE_ENABLED,RESOURCE_AVAILABLE). 

---

**Вариант 2**

**Система контроля доступа**. Сформировать в хост-подсистеме и передать в SPE структуры: STORAGE0 и STORAGE1. В структуре STORAGE0 хранятся id записей в хранилище 0 (1M записей). В структуре STORAGE1 хранятся id записей в хранилище 1 (2M записей).  По запросу пользователя выдать те записи, которые присутствуют в обеих структурах. Для реализации запроса использовать команду объединения множеств lnh_or_async(INTERSECTION,RESOURCE_ENABLED,RESOURCE_AVAILABLE). 
